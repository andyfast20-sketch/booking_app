...
<div id="panel">
  ...
  <h2 style="margin-top:2rem;">📗 Current Free Slots</h2>
  <div id="avail" class="grid"></div>

  <h2 style="margin-top:2rem;">📘 Booked Appointments</h2>
...
async function loadAvailability(){
  ...
  }catch(e){c.innerHTML="<p>❌ Error loading slots.</p>";}
}
async function loadBookings(){
  ...
  try{
    ...
    data.bookings.forEach(b=>{
      const div=document.createElement("div");
      div.className="card";
      div.innerHTML=`<h3>${b.name||"Unknown"}</h3><p><i class='fa-regular fa-clock'></i> ${b.time||"–"}</p>`;
      c.appendChild(div);
    });
